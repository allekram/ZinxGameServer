...
func (m *AOIManager) GetSuroundGridsByGid(gID int) (grids []*Grid) {
	...
	//判断idx编号是否左边还有格子，如果有 放到gidsX 集合中
	if idx > 0 {
...
func (m *AOIManager) GetPidsByPos(x, y float32) (playerIDs []int) {
	...
	//通过GID得到周边九宫格信息
	grids := m.GetSuroundGridsByGid(gID)
	//将九宫格的信息里的全部的Player的id累加到playerIDs
	for _, gird := range grids {
		playerIDs = append(playerIDs, gird.GetPlayerIDs()...)
		fmt.Printf("=======>grid ID : %d, pids : %v ======", gird.GID, gird.GetPlayerIDs())
	}
	return
}
